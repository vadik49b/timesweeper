...
export default function AvailabilityGrid(props: Props) {
				...
				(_, rowOffset) =>
					Array.from(
						...
						(_, colOffset) => {
							...
							const nextSlot =
								day && time
									? props.slotByDayTime[`${day.key}|${time.key}`]
									: undefined;
							...
							return {
								...
								currentValue,
								currentGlyph:
									currentValue === undefined ? null : statusGlyph(currentValue),
								nextGlyph:
									bounds.targetValue === 0
										? null
										: statusGlyph(bounds.targetValue),
								showNextCentered:
									currentValue === undefined && bounds.targetValue !== 0,
							};
						},
					),
	...
	const updateDragTarget = (clientX: number, clientY: number) => {
		...
		const nextCol = snapIndex(
			clientX - gridRect.left,
			gesture.colLefts,
			gesture.cellWidth,
		);
		const nextRow = snapIndex(
	...
	const startAutoScrollLoop = () => {
		const step = () => {
			...
			if (horizontalScrollContainer) {
				...
				if (
					leftDistance < edgeThreshold &&
					horizontalScrollContainer.scrollLeft > 0
				) {
					deltaX = -Math.ceil(
						((edgeThreshold - leftDistance) / edgeThreshold) * 18,
					);
				} else if (
					...
					deltaX = Math.ceil(
						((edgeThreshold - rightDistance) / edgeThreshold) * 18,
					);
				}
			}
			...
			if (topDistance < edgeThreshold) {
				deltaY = -Math.ceil(
					((edgeThreshold - topDistance) / edgeThreshold) * 18,
				);
			} else if (bottomDistance < edgeThreshold) {
				deltaY = Math.ceil(
					((edgeThreshold - bottomDistance) / edgeThreshold) * 18,
				);
			}
					...
					>
						...
						<span class="availability-grid__day-date">
							<Show when={day.showMonthLabel}>
								<span class="availability-grid__day-month">
									{day.monthLabel}
								</span>
							</Show>
									...
									onPointerDown={(event) => {
										...
										setDragGesture({
											...
											colLefts: props.days.map(
												(_, index) =>
													cellRefs[colAnchorRow]?.[index]?.offsetLeft ??
													firstCell.offsetLeft,
											),
											rowTops: props.times.map(
												(_, index) =>
													cellRefs[index]?.[rowAnchorCol]?.offsetTop ??
													firstCell.offsetTop,
											),
				...
				{(overlay) => (
					<div
						classList={{
							...
							"availability-grid__drag-overlay--yes":
								overlay().targetValue === 1,
							"availability-grid__drag-overlay--maybe":
								overlay().targetValue === 2,
							"availability-grid__drag-overlay--no":
								overlay().targetValue === 0,
						}}
						style={{
									...
									<Show when={marker.nextGlyph}>
										<span
											classList={{
												...
												"availability-grid__drag-marker-new--centered":
													marker.showNextCentered,
											}}
										>
											{marker.nextGlyph}
										</span>
